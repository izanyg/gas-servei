      ...
      <tbody>
        ...
        do_action( 'woocommerce_grouped_product_list_before', $grouped_product_columns, $quantites_required, $product );
          ...
          foreach( $grouped_product_columns as $column_id ){
            switch ( $column_id ) {
              case 'quantity':
                echo '<th id="th-qtt">Cantidad</th>';
                break;
              case 'label':
                echo '<th id="th-name">Nombre</th>';
                break;
              case 'price':
                echo '<th id="th-price">Precio</th>';
                break;
              default:
                $term =  get_taxonomy( $column_id );
                if( !is_wp_error($term) && is_object($term)){
                  echo '<th>' . $term->labels->singular_name . '</th>';
                }
                break;
            }
          }
        ...
        foreach ( $grouped_products as $grouped_product_child ) {
          ...
          $product_attributes = $grouped_product_child->get_attributes();
          ...
          foreach ( $grouped_product_columns as $column_id ) {
            ...
            switch ( $column_id ) {
              ...
              default:            
                $term =  get_taxonomy( $column_id );
                if( !is_wp_error($term) && is_object($term)){
                  $attr_value = $grouped_product_child->get_attribute($column_id);
                  $value = '<p class="attribute">' . $attr_value . '</p>';
                }
                else{
                  continue 2;
                }
