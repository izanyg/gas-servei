<?php
/*
Plugin Name: Gas Servei Variations Table
Description: (FORKED) Show a table of all the available variations of a variable product to make it easier for users to find the variation they are searching for.
...
function woo_variations_table_get_columns_labels()
{
    return array(
        ...
        'sku' => __('Reference', 'woo-variations-table'),
                ...
                <tr valign="top">
                    ...
                    <td>
                        <select name='woo_variations_table_place'>
                            <option value="woocommerce_after_single_product_summary_1" <?php echo $place == 'woocommerce_after_single_product_summary_9' ? "selected" : ''; ?>>After product summary ( before description )</option>
                            ...
                            <option value="woocommerce_after_single_product_summary_data_tabs_1" <?php echo $place == 'woocommerce_after_single_product_summary_data_tabs_1' ? "selected" : ''; ?>>Before shopkeeper tabs</option>
    ...
    if ($product->is_type('variable')) {
        ...
        foreach ($variation_attributes as $key => $name) {
            ...
            for ($i = 0; count($name) > $i; $i++) {
                ...
                $slug = $slug[$i];
                if ($term) {
                    array_push($options, array('name' => __($term->name, 'gasservei'), 'slug' => $slug));
                } else {
        ...
        $activeColumns['price_html'] = 'off';
        $activeColumns['regular_price_html'] = 'on';
        $activeColumns['sale_price_html'] = 'on';
        array_splice($columnsOrder, array_search('price_html', $columnsOrder), 1, ['regular_price_html', 'sale_price_html']);
        ...
        $woo_variations_table_data = array(
            "variations" =>  gs_get_tipologias_variations($variations),
            "attributes" => $attrs,
            ...
            "textVars" => array(
                ...
                "addToCartText" => __("Add", 'woo-variations-table'),
                "anyText" => __("Any", 'woo-variations-table'),
                "searchPlaceholderText" => __("Filter references", 'woo-variations-table'),
